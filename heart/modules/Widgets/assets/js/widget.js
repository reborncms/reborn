	...
	$('.widget_add_btn').on('click', function(e){
		...
		$.ajax({
		  	...
		  	success : function(data) {
		  		...
		  		if (obj.status == 'ok') {
		  			...
		  			$.ajax({
		  				...
		  				success : function(data) {
		  					...
		  					if (dt.status == 'ok') {
		  						var setting_btn = '<a href="'+ SITEURL + ADMIN + '/widget/settings/' + widget_name + '/' +  obj.id + '" class="widget_settings_btn"><i class="icon-setting icon-grey widget-ico"></i></a>';
		  		...
		  		$('#msg_area').animate({opacity: 1.0},4000).fadeOut('slow');
